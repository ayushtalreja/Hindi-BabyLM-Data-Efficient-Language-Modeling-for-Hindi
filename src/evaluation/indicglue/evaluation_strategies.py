...
class BinaryCandidateStrategy(EvaluationStrategy):
    """
    Strategy for binary per-candidate evaluation (DEPRECATED).

    This approach evaluates each candidate choice independently as a binary
    classification problem. It's kept for backward compatibility but is
    deprecated in favor of the MultipleChoiceWrapper approach.

    For each example with N choices:
    - Run N forward passes (one per candidate)
    ...
    def __init__(self, device: torch.device, tokenizer, max_length: int, data_extractor):
        """
        Initialize BinaryCandidateStrategy
        ...
        logger.warning(
            "BinaryCandidateStrategy is deprecated. "
            "Use MultipleChoiceStrategy with MultipleChoiceWrapper instead."
        )
    ...
    def evaluate(
        ...
        **kwargs
        ...
        """
        Evaluate using binary per-candidate scoring.

        Args:
            model: Binary classification model (2 output classes)
        ...
        Returns:
            Dict with 'predictions' and 'labels' lists
        """
        logger.info(f"Evaluating {task_name} using binary per-candidate scoring (DEPRECATED)")
        ...
        for example in tqdm(dataset, desc=f"Evaluating {task_name}"):
            # Extract context and choices
            context = self.data_extractor.extract_text(example, task_name)
            choices = self.data_extractor.extract_choices(example, task_name)
            label = self.data_extractor.extract_label(example, task_name)

            # Create inputs for all candidates
            candidate_texts = []
            for choice in choices:
                # Format: "context [SEP] candidate"
                text = f"{context} [SEP] {choice}"
            ...
            for candidate_text in candidate_texts:
                ...
                with torch.no_grad():
                    ...
                    # Get probability of "correct" (class 1)
                    # logits shape: [1, 2] for binary classification
                    probs = torch.softmax(logits, dim=-1)
                    score = probs[0, 1].item()  # Probability of class 1
                    candidate_scores.append(score)

            # Predict candidate with highest score
            pred = np.argmax(candidate_scores)
            predictions.append(pred)
            labels.append(label)
