...
class DataLoaderFactory:
    ...
    def create_binary_candidate_dataloader(
        self,
        dataset: Dataset,
        task_name: str,
        shuffle: bool = False,
        ...
        """
        Create dataloader for binary per-candidate evaluation (DEPRECATED).
        ...
        Returns:
            DataLoader for binary per-candidate evaluation
        """
        logger.warning(
            f"Binary per-candidate evaluation is deprecated. "
            f"Use MultipleChoiceWrapper instead for {task_name}."
        )

        def collate_fn_binary(examples):
            """
            Collate function for binary per-candidate evaluation.

            Evaluates each choice as separate binary classification.
            """
            all_texts = []
            all_labels = []
            ...
            for example in examples:
                ...
                context = self.data_extractor.extract_text(example, task_name)
                choices = self.data_extractor.extract_choices(example, task_name)
                correct_idx = self.data_extractor.extract_label(example, task_name)

                # Create binary examples: (context, choice, is_correct)
